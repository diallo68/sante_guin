...
export default function DemandesProPage() {
  ...
  const saveNote = async () => {
    ...
    await fetch('/api/admin/subscription-requests', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: selected._id, adminNote: note }),
    });
    setSaving(false);
    fetchRequests();
  };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">Note interne</label>
                <textarea
                  value={note}
                ...
                />
                <button onClick={saveNote} disabled={saving} className="mt-1 text-xs text-teal-600 hover:underline disabled:opacity-50">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    { status: 'contacted', label: 'Marquer Contacté', color: 'bg-blue-100 text-blue-700 hover:bg-blue-200' },
                    { status: 'active', label: 'Activer l\'abonnement', color: 'bg-green-100 text-green-700 hover:bg-green-200' },
                    { status: 'rejected', label: 'Refuser', color: 'bg-red-100 text-red-700 hover:bg-red-200' },
                    { status: 'pending', label: 'Remettre en attente', color: 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200' },
                  ].map(action => (
                    <button
                      key={action.status}
                      onClick={() => updateStatus(selected._id, action.status)}
                      disabled={saving || selected.status === action.status}
                      className={`px-4 py-2 rounded-xl text-xs font-bold transition disabled:opacity-40 ${action.color}`}
                    >
                      {action.label}
                    </button>
                  ))}
