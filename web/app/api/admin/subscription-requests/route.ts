...
// Statuts autorisés — voir le schéma Mongoose (models/SubscriptionRequest.ts).
// `findByIdAndUpdate` n'exécute pas les validateurs du schéma par défaut :
// sans ce contrôle explicite, un statut arbitraire pouvait être enregistré
// et lu ensuite par du code qui suppose l'un des quatre statuts connus.
const VALID_STATUSES = ['pending', 'contacted', 'active', 'rejected'] as const;
...
export async function PATCH(req: NextRequest) {
  ...
  if (sendMessageOnly) {
    ...
    if (previous.email && adminNote?.trim()) {
      // `nom` et `planName` viennent du formulaire public (POST /api/pro/subscribe,
      // non ré-échappés en base) et `adminNote` est du texte libre saisi par
      // l'admin : les trois sont interpolés tels quels dans ce template HTML
      // sans échappement — une balise dans l'un de ces champs pouvait
      // falsifier le contenu de l'email envoyé au nom de la plateforme
      // (mise en forme trompeuse, lien de phishing) — voir audit S19/RA-14.
      const firstName = escapeHtml(previous.nom.split(' ')[0]);
      const planName = escapeHtml(previous.planName);
      const noteHtml = escapeHtml(adminNote);
      await sendEmail({
        to: previous.email,
        subject: `Message de l'équipe Mondocteur — Offre ${planName}`,
        html: `
          <div style="font-family:sans-serif;max-width:560px;margin:auto;padding:32px;background:#f9fafb;border-radius:16px">
            ...
            <p style="color:#6b7280;font-size:13px;margin-bottom:8px">Concernant votre demande d'abonnement <strong style="color:#0d9488">Offre ${planName}</strong> :</p>
            <div style="background:#fff;border-left:4px solid #0d9488;padding:16px;border-radius:0 10px 10px 0;margin-bottom:20px">
              <p style="color:#374151;font-size:14px;margin:0;line-height:1.7">${noteHtml}</p>
            </div>
  ...
  if (statusChanged && updated.email && (status === 'active' || status === 'rejected')) {
    ...
    // `nom`/`planName` viennent du formulaire public non ré-échappé en base
    // et `adminNote` est saisi librement par l'admin : les trois doivent être
    // échappés avant interpolation dans ces templates HTML — voir S19/RA-14
    // (même raisonnement que pour l'email "sendMessageOnly" ci-dessus).
    const firstName = escapeHtml(updated.nom.split(' ')[0]);
    const planName = escapeHtml(updated.planName);
    const noteHtml = adminNote ? escapeHtml(adminNote) : '';

    if (status === 'active') {
      await sendEmail({
        to: updated.email,
        subject: `✅ Votre abonnement Mondocteur Pro ${planName} est activé !`,
        html: `
          <div style="font-family:sans-serif;max-width:560px;margin:auto;padding:32px;background:#f9fafb;border-radius:16px">
            ...
            <p style="color:#4b5563;line-height:1.7;margin-bottom:20px">
              Votre abonnement <strong style="color:#0d9488">Mondocteur Pro — Offre ${planName}</strong> a été validé et activé par notre équipe.
            </p>
        ...
        html: `
          <div style="font-family:sans-serif;max-width:560px;margin:auto;padding:32px;background:#f9fafb;border-radius:16px">
            ...
            <p style="color:#4b5563;line-height:1.7;margin-bottom:16px">
              Nous avons bien reçu votre demande d'abonnement <strong>Mondocteur Pro — Offre ${planName}</strong>. Malheureusement, nous ne sommes pas en mesure de la traiter pour le moment.
            </p>
